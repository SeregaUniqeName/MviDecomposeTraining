...
import com.example.mvidecomposetraining.R
...
import com.example.mvidecomposetraining.presentation.utils.formattedFullDate
import com.example.mvidecomposetraining.presentation.utils.formattedShortDayOfWeek
import com.example.mvidecomposetraining.ui.theme.CardGradients
import tempToFormattedString
...
private fun Forecast(forecast: Forecast) {
    Column(
        ...
        Row(
            ...
            Text(
                text = forecast.current.tempC.tempToFormattedString(),
                style = MaterialTheme.typography.headlineLarge.copy(fontSize = 70.sp)
            )
            GlideImage(
                modifier = Modifier.size(90.dp),
                model = forecast.current.conditionUrl,
                contentDescription = null
            )
        }
        Text(
            text = forecast.current.date.formattedFullDate(),
            style = MaterialTheme.typography.titleLarge
        )
        Spacer(modifier = Modifier.weight(1f))
